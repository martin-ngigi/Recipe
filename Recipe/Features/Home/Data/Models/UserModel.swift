...
class UserSwiftData {
    ...
    convenience init(model: UserModel) {
        ...
        let rateSwiftData = model.rate.map { TotalRateSwiftData(model: $0) }
...
struct UserModel: Codable, Hashable {
    ...
    var allRates: [AllRatingModel]?
    var rate: TotalRateModel?
    ...
    init(swiftData: UserSwiftData) {
        ...
        let rateModel = swiftData.rate.map { TotalRateModel(swiftData: $0) }
