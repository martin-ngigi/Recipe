...
struct FirebaseAuthRepository: CreateFirebaseUserRepositoryProtocol, LoginFirebaseRepositoryProtocol,
    GoogleFirebaseAuthRepositoryProtocol, ResetPasswordFirebaseRepositoryProtocol, LogoutFirebaseRepositoryProtocol,
    DeleteFirebaseAccountRepositoryProtocol
{
        ...
        do {
            ...
            */
            ...
            if let authErrorCode = AuthErrorCode.Code(rawValue: err.code) {
                ...
                case .wrongPassword:
                    os.Logger().debug("wrong password")
                case .invalidEmail:
                    os.Logger().debug("invalid email")
                default:
                    os.Logger().debug("unknown error")
                }
            }
            */
    ...
    func googleAuth() async -> Result<AuthDataResult, FirebaseAuthError> {
        do {
            ...
            guard let idToken = user.idToken?.tokenString else {
                os.Logger().debug("DEBUG: googleAuth Error no idToken")
            ...
            os.Logger().debug("Google Auth success. OpenID: \(authResult.user.uid)")
            return .success(authResult)
        }
        catch {
            os.Logger().debug("Error: Failed to create google auth user with error \(error.localizedDescription)")
    ...
    func resetPassword(email: String) async -> Result<Bool, FirebaseAuthError> {
        do {
            ...
            os.Logger().debug("Error: Failed to reset Password with error \(error.localizedDescription)")
            return .failure(FirebaseAuthError.custom("Error: \(error.localizedDescription)"))
        }
    }
    ...
    func logout() -> Result<Bool, FirebaseAuthError> {
        do {
            ...
            os.Logger().debug("Error: Failed to logout with error \(error.localizedDescription)")
            return .failure(FirebaseAuthError.custom("Error: \(error.localizedDescription)"))
        }
    }
    ...
    func deleteAccount() async -> Result<Bool, FirebaseAuthError> {
        do {
            ...
            os.Logger().debug("Error: Failed to delete with error \(error.localizedDescription)")
            return .failure(FirebaseAuthError.custom("Error: \(error.localizedDescription)"))
        }
    }
